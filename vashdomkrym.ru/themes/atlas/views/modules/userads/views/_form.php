...
Yii::app()->clientScript->registerScriptFile( Yii::app()->theme->baseUrl. '/js/AMap.js' );
					...
					if($video->isFile()){
						if($video->isFileExists()){
							...
							Yii::app()->clientScript->registerScript('player-'.$video->id.'', '
									flowplayer("player-'.$video->id.'", "'.Yii::app()->theme->baseUrl."/js/flowplayer/flowplayer-3.2.16.swf".'",
									{
										clip: {
											autoPlay: false
										},
...
<?php
if (issetModule('seo') && !$model->isNewRecord && $model->active != Apartment::STATUS_DRAFT  && (param('allowUserSeo', 1) || Yii::app()->user->checkAccess('backend_access'))) {
	echo '<br />';
