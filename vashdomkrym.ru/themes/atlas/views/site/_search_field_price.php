<div class="<?php echo $divClass; ?>">
	...
	if (issetModule('selecttoslider') && param('usePriceSlider') == 1) { ?>
		...
		<?php if($this->searchShowLabel){ ?>
			<div class="<?php echo $textClass; ?>"><?php echo tc('Price'); ?>:</div>
		<?php } ?>

		<div class="<?php echo $controlClass; ?>">
			<input onblur="changeSearch();" type="number" id="priceMin" name="price_min" class="width120 search-input-new" placeholder="<?php echo tc('Price from') ?>" value="<?php echo CHtml::encode($priceMinSel); ?>"/>&nbsp;
			<input onblur="changeSearch();" type="number" id="priceMax" name="price_max" class="width120 search-input-new" placeholder="<?php echo tc('Price to') ?>" value="<?php echo CHtml::encode($priceMaxSel); ?>"/>&nbsp;
